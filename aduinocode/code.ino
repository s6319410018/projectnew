...
void loop() {
  ...
  httpESPKEY.addHeader("Content-Type", "application/x-www-form-urlencoded");  // Specify content-type header
  int httpCodeESPKEY = httpESPKEY.POST(postESPKEY);
  Serial.print(postESPKEY);
  ...
  if (httpCodeESPKEY == 200) {
    ...
    if (GET_control_AI == "1") {
      ...
      if (pressuretostring > GET_result_SD) {
        digitalWrite(solenoid_control, HIGH);
        Serial.println("Ai กำลังปิดน้ำ");
        Solenoid = 0;
      } else {
        if(GET_control_Solenoid == "1"){
        digitalWrite(solenoid_control, LOW);
        Serial.println("กำลังเปิดน้ำตอน Ai ทำงาน");
        Solenoid = 1;
        while (GET_control_Date_ON == formatted_date && GET_control_Time_ON == formatted_time) {
              digitalWrite(solenoid_control, LOW);
              Serial.println("ตั้งเวลาเปิดน้ำสำเร็จ");
              timecontrolstop = 1;
              time_control = 1;
              Solenoid = 1;
          ...
          if (httpCodeESPKEY == 200) {
            ...
            Serial.print("motionDisplay = ");  // พิมพ์ข้อมความส่งเข้าคอมพิวเตอร์ "val = "
            Serial.println(motionDisplay);
            if (motionDisplay == 1) {  //หน่วงเวลาที่อุปกรณ์ 15 millisec
              display.display();
              display.setTextSize(1);
              display.setTextColor(SH110X_WHITE);
              display.setCursor(0, 0);
            ...
            if (GET_control_Date_OFF == formatted_date && GET_control_Time_OFF == formatted_time || GET_control_Date_ON == "0000-00-00" && GET_control_Date_OFF == "0000-00-00" && GET_control_Time_ON == "00:00:00" && GET_control_Time_OFF == "00:00:00") {
              digitalWrite(solenoid_control, HIGH);
              Serial.println("ตั้งเวลาปิดน้ำสำเร็จ");
              time_control = 0;
              Solenoid = 0;
              break;
            }
        ...
        Solenoid = 0;
        digitalWrite(solenoid_control, HIGH);
        Serial.println("กำลังปิดน้ำตอน Ai ทำงาน");
        while (GET_control_Date_ON == formatted_date && GET_control_Time_ON == formatted_time) {
              digitalWrite(solenoid_control, LOW);
              Serial.println("ตั้งเวลาเปิดน้ำสำเร็จ");
              timecontrolstop = 1;
        ...
        if(GET_control_Solenoid == "1"){
        digitalWrite(solenoid_control, LOW);
        Serial.println("กำลังเปิดน้ำกำลังปิดน้ำตอน Ai ไม่ทำงาน");
        Solenoid = 1;
        while (GET_control_Date_ON == formatted_date && GET_control_Time_ON == formatted_time) {
              digitalWrite(solenoid_control, LOW);
              Serial.println("ตั้งเวลาเปิดน้ำสำเร็จ");
        ...
        Solenoid = 0;
        digitalWrite(solenoid_control, HIGH);
        Serial.println("**** กำลังปิดน้ำตอน Ai ไม่ทำงาน ****");
        for(int n;GET_control_Date_ON == formatted_date && GET_control_Time_ON == formatted_time;n++){
                    digitalWrite(solenoid_control, LOW);
              Serial.println("กำลังเปิดน้ำตอน Ai ไม่ทำงาน");
              timecontrolstop = 1;
              time_control = 1;
              Solenoid = 1;
          ...
          if (httpCodeESPKEY == 200) {
           ...
           if (GET_control_Date_OFF == formatted_date && GET_control_Time_OFF == formatted_time || GET_control_Date_ON == "0000-00-00" && GET_control_Date_OFF == "0000-00-00" && GET_control_Time_ON == "00:00:00" && GET_control_Time_OFF == "00:00:00") {
              digitalWrite(solenoid_control, HIGH);
              Serial.println("ลูปหยุดการทำงาน");
              time_control = 0;
              Solenoid = 0;
              break;
            }
      ...
      /*  while (GET_control_Date_ON == formatted_date && GET_control_Time_ON == formatted_time) {
          ...
          if (httpCodeESPKEY == 200) {
            ...
            if (GET_control_Date_OFF == formatted_date && GET_control_Time_OFF == formatted_time || GET_control_Date_ON == "0000-00-00" && GET_control_Date_OFF == "0000-00-00" && GET_control_Time_ON == "00:00:00" && GET_control_Time_OFF == "00:00:00") {
              digitalWrite(solenoid_control, HIGH);
              Serial.println("ลูปหยุดการทำงาน A0 S0");
              time_control = 0;
              Solenoid = 0;
              break;
            }
